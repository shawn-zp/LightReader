...
public class PageWidget extends View {
	...
	float curPageBitmapX;

	public PageWidget(PageActivity pageActivity) {
		super(pageActivity);
	...
	public boolean doTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			mTouch.y = event.getY();
			curPageBitmapX = 0;
		}else if (!pageActivity.pagePreferences.isTranslation() && event.getAction() == MotionEvent.ACTION_MOVE) {
			mTouch.x = event.getX();
			mTouch.y = event.getY();
			this.postInvalidate();
	...
	protected void onDraw(Canvas canvas) {
		...
		if(pageActivity.pagePreferences.isTranslation()) {
			...
			drawCurrentPageTranslation(canvas, mCurPageBitmap);
		}else {
			calcPoints();
	...
	private void drawCurrentPageTranslation(Canvas canvas, Bitmap bitmap) {
		if(curPageBitmapX <= -mWidth || curPageBitmapX >= mWidth)
			return;

		canvas.drawBitmap(bitmap, curPageBitmapX, 0, null);
		if(mTouch.x > mWidth*0.5)
			curPageBitmapX -= 30;
		else
			curPageBitmapX += 30;
	...
	public void computeScroll() {
		super.computeScroll();
		if (!pageActivity.pagePreferences.isTranslation() && mScroller.computeScrollOffset()) {
